...
export default function PreliminaryInventory({ inventoryId, onNavigate }: PreliminaryInventoryProps) {
  ...
  const loadEntries = async (reset: boolean = false) => {
    ...
    try {
      ...
      // Przekazujemy limit 50 i aktualny offset
      const data = await entryService.getPreliminaryEntries(
        inventoryId, 
        selectedCategory, 
        50, 
        currentOffset
  ...
  const loadMoreSuggestions = async () => {
    ...
    const moreSuggestions = await productService.search(newEntry.product_name, newEntry.category_id || selectedCategory, 50, newOffset);
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2 bg-gray-100 dark:bg-gray-700 px-4 py-2 rounded-md">
              ...
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                W bazie (kategoria): <span className="font-bold text-gray-900 dark:text-white">{categoryProductCount}</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white">
            Wpisy w tej kategorii ({entries.length})
          </h3>
        </div>
        ...
        >
          ...
          {loadingMore && (
            <div className="flex justify-center py-4">
              <LoadingSpinner size="sm" text="Wczytywanie kolejnych wpisów..." />
            </div>
          )}
        </div>
        ...
        <div className="bg-gray-50 dark:bg-gray-700 px-6 py-3 border-t border-gray-200 dark:border-gray-600 text-right">
          <span className="text-sm font-bold text-gray-900 dark:text-white">
            Suma widocznych pozycji: {totalValue.toFixed(2)} zł
          </span>
        </div>
      </div>
